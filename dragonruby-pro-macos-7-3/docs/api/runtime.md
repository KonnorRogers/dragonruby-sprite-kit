...
increases, the following approaches will keep `Object` from being
polluted and give you a bit more control over the entry point of your
game.

#### `self` Scoping

Defining your top level functions prefixed with `self` will keep
`Object` from being polluted with these function. Example:
...
This ensures that `Object.respond_to? :tick` returns `false` (which is
a good thing because you don't want every object to have a `tick` function).

#### `Main` Scoping

For more control over DragonRuby's entry point. You can define `boot`,
`tick`, `reset`, etc on `Main`. Example:

```ruby
# using the Main module removes the need to prefix
# every function with `self.`
module Main
  def boot args
    @game_launched_at = Time.now
  end
